...
// Находит позиции подписей номеров помещений на чертеже.
// Возвращает [{ number, x, y }] — нормализованные координаты 0..1.
export async function extractRoomPositions(base64image, mediaType = 'image/jpeg') {
  return callClaude({
    base64image, mediaType, maxTokens: 1024,
    prompt: `На изображении — поэтажный план этажа.
Найди все цифровые подписи (номера) помещений, написанные внутри каждой комнаты.
Определи положение центра каждой подписи относительно размеров изображения.

Верни ТОЛЬКО JSON-массив без markdown:
[{"number":"1","x":0.35,"y":0.40}]

Правила:
- number: точный номер (включая суффиксы: 1а, 2б, 9б, 10а и т.д.)
- x: от 0.0 (левый край) до 1.0 (правый край)
- y: от 0.0 (верхний край) до 1.0 (нижний край)
- Координаты — центр подписи номера внутри помещения
- Пропусти нечёткие или внешние подписи
- НЕ включай площади, названия, итоговые строки — только number, x, y`,
  })
}
